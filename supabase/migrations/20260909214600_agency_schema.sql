-- FRAME/SHIFT initial database migration.
-- Public visitors may read published portfolio entries only.
-- Enquiries are accepted only through the server-side service/secret-key RPC.
begin;
...
begin
  ...
  insert into public.agency_enquiries (
    id, reference, name, email, company, production_type,
    deliverables, budget, timeline, brief, ip_hash
  ) values (
    p_request_id, enquiry_reference, btrim(p_name), lower(p_email), btrim(p_company),
    p_production_type, p_deliverables, p_budget, p_timeline, btrim(p_brief), p_ip_hash
  );
